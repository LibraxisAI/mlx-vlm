...
def get_class_predicate(
    ...
    quantization_cfg = quantization if quantization is not None else quantization_config

    def _predicate(path, module):
        # Skip legacy multimodal layers unless the checkpoint has quantized
        # tensors for this exact module.
        if (
            skip_multimodal_module(path)
            and skip_vision
            and not _has_quantized_weights(path, weights)
        ):
            return False
        # Skip 1-bit layers already replaced by custom modules.
        if (
            quantization_cfg is not None
            and _quantization_for_path(quantization_cfg, path).get("bits") == 1
        ):
            return False
        # Config keys from underlying text checkpoints can omit the
        # ``language_model.`` wrapper prefix.
        override = None
        if quantization_cfg is not None:
            override = quantization_cfg.get(path)
            if override is None and path.startswith("language_model."):
                override = quantization_cfg.get(path[len("language_model.") :])
        ...
        if hasattr(module, "weight") and module.weight.size % 64 != 0:
            return False
        return weights is None or f"{path}.scales" in weights
...
def load_processor(
    ...
    try:
        processor = AutoProcessor.from_pretrained(
            model_path, use_fast=False, trust_remote_code=True, **kwargs
        )
